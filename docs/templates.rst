...
The default template located at ``forms/form_block.html`` can be seen below:

.. literalinclude:: ../wagtailforms/templates/forms/form_block.html
...
This is as simple as adding it to the ``WAGTAILFORMS_FORM_TEMPLATES`` in settings as below.
...
.. code-block:: python
    ...
    WAGTAILFORMS_FORM_TEMPLATES = (
        ('forms/form_block.html', 'Default Form Template'),  # default
...
package out of the box. If you dont want it just remove it from the ``WAGTAILFORMS_FORM_TEMPLATES`` setting.
...
``forms/non_existent_form.html``

.. literalinclude:: ../wagtailforms/templates/forms/non_existent_form.html

You can override this by putting a copy of the template in you own project using the same 
path under a templates directory ie ``app/templates/forms/non_existent_form.html``. As long as the app is before
``wagtailforms`` in ``INSTALLED_APPS`` it will use your template instead.
