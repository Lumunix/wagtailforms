...
from wagtailforms import hooks as form_hooks
...
from wagtailforms.utils.requests import get_form_instance_from_request
...
class FormURLHelper(AdminURLHelper):
    def get_action_url(self, action, *args, **kwargs):
        if action in ["advanced", "copy", "submissions"]:
            return reverse("wagtailforms:forms_%s" % action, args=args, kwargs=kwargs)
...
class InstanceSpecificViewHookMixin:
    """Mixin class responsible to apply wagtailforms specific hooks."""
...
def register_admin_urls():
    from wagtailforms import urls

    return [path("wagtailforms/", include((urls, "wagtailforms")))]
...
def process_form(page, request, *args, **kwargs):
    ...
    # only process if settings.WAGTAILFORMS_ENABLE_FORM_PROCESSING is True
    ...
    if request.method == "POST":
        ...
        if form_def:
            ...
            if form.is_valid():
                ...
                context.update(
                    {
                        "invalid_form_reference": form.data.get("form_reference"),
                        "invalid_form": form,
                    }
                )
...
class WagtailFormsChooserViewSet(ModelChooserViewSet):
    icon = "form"
    model = Form
