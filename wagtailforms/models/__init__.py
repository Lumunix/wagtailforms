...
def get_form_model_string():
    ...
    """
    return getattr(settings, "WAGTAILFORMS_FORM_MODEL", "wagtailforms.Form")


def get_form_model():
    """
    Get the form model from the ``WAGTAILFORMS_FORM_MODEL`` setting.
    Useful for developers making Wagtail plugins that need the form model.
    Defaults to the standard :class:`~wagtailforms.Form` model
    ...
    try:
        ...
        raise ImproperlyConfigured(
            "WAGTAILFORMS_FORM_MODEL must be of the form 'app_label.model_name'"
        )
    except LookupError:
        raise ImproperlyConfigured(
            "WAGTAILFORMS_FORM_MODEL refers to model '%s' that has not been installed"
            % model_string
        )
